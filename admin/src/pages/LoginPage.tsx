      ...
      <main className="grid h-full grid-rows-[39%_61%] lg:grid-cols-[1.16fr_.84fr] lg:grid-rows-1">
        <section className="relative min-h-0 overflow-hidden border-b border-white/10 lg:border-b-0 lg:border-r">
          ...
          <div className="absolute left-4 top-4 z-10 flex items-center gap-2.5 sm:left-7 sm:top-6">
            <div><p className="text-sm font-extrabold">Flodata Admin</p><p className="text-[9px] font-bold uppercase tracking-[.2em] text-accent-300">Operations</p></div>
          </div>

          {hotspots.map((point, index) => <button key={index} onClick={() => setStep(index)} aria-label={`View ${steps[index].title}`} className={`absolute z-10 grid h-8 w-8 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border transition-all duration-300 ${step === index ? "scale-110 border-white/70 bg-brand-500 shadow-[0_0_0_8px_rgba(99,102,241,.18),0_0_28px_rgba(99,102,241,.9)]" : "border-white/50 bg-slate-950/55 hover:scale-110 hover:bg-brand-500"}`} style={point}><span className="h-1.5 w-1.5 rounded-full bg-white"/></button>)}
          ...
          <div className="absolute bottom-4 left-4 right-4 z-10 hidden items-end justify-between gap-4 sm:flex sm:bottom-6 sm:left-7 sm:right-7">
            <div className="max-w-xl"><h1 className="text-3xl font-black leading-none tracking-[-.045em] lg:text-5xl">Your building,<br/><span className="bg-gradient-to-r from-brand-200 to-cyan-300 bg-clip-text text-transparent">alive with context.</span></h1></div>
          ...
          <div className="w-full max-w-[520px] rounded-[26px] border border-white/10 bg-white/[.065] p-1.5 shadow-[0_28px_90px_rgba(0,0,0,.35)] backdrop-blur-xl">
            <div className="rounded-[21px] border border-white/[.07] bg-[#0d162b]/95 p-4 sm:p-5 lg:p-7">
              <div className="flex items-center justify-between">
                <div className="flex gap-1.5">{steps.map((_, index) => <button key={index} onClick={() => setStep(index)} aria-label={`Step ${index + 1}`} className={`h-1.5 rounded-full transition-all ${step === index ? "w-8 bg-gradient-to-r from-brand-400 to-cyan-300" : "w-4 bg-white/15 hover:bg-white/30"}`}/>)}</div>
              ...
              <div key={step} className="login-step-enter">
                <div className="mt-4 flex items-start gap-3 lg:mt-6"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 text-lg shadow-lg shadow-brand-950/50">{active.icon}</span><div><p className="text-[9px] font-black uppercase tracking-[.19em] text-brand-300">{active.eyebrow}</p><h2 className="mt-1 text-xl font-black tracking-tight sm:text-2xl lg:text-3xl">{active.title}</h2></div></div>
