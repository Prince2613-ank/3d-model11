...
export function DataTable<T>({ columns, rows, keyField, isLoading, emptyMessage = "No records found.", onRowClick }: DataTableProps<T>) {
  if (isLoading) return <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-900"><TableSkeleton columns={Math.min(columns.length, 4)} /></div>;
  if (rows.length === 0) return <div className="rounded-xl border border-dashed border-slate-200 bg-white px-4 py-12 text-center text-sm font-semibold text-slate-400 dark:border-white/10 dark:bg-slate-900/60">{emptyMessage}</div>;

  return (
    <>
      <div className="space-y-3 md:hidden">
        {rows.map((row) => <article key={keyField(row)} onClick={() => onRowClick?.(row)} onKeyDown={(event) => { if (onRowClick && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onRowClick(row); } }} role={onRowClick ? "button" : undefined} tabIndex={onRowClick ? 0 : undefined} className={`overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)] dark:border-white/10 dark:bg-slate-900 ${onRowClick ? "cursor-pointer transition hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500/40" : ""}`}>
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] dark:border-white/10 dark:bg-slate-900 md:block">
